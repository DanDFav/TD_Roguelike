...
func _ready() -> void:
	...
	HALF_SHIFT_X = len(map[0]) / 2
	HALF_SHIFT_Z = len(map) / 2
	create_map()
	child.object_inst.recieve_path_info(path_nodes)
...
func create_new_special(x: int, y: int, z:int, length: Vector2): 
	...
	var starting_x = x * BLOCK_SIZE - 0.1 + 0.5
	var starting_z = z * BLOCK_SIZE + 0.1 + 0.5
...
func create_new_range_block(x: int, y: int, z: int, row_inst: Node3D, length:Vector2 ,center: bool): 
	...
	range_block.position = Vector3(starting_x - centering_val_x, (1 * BLOCK_SIZE + 0.1), starting_z - centering_val_z)
	
	RB_mesh_inst.size = Vector3(BLOCK_SIZE, BLOCK_SIZE + 0.2, BLOCK_SIZE)
...
func create_new_ground_block(x: int, y: int, z: int, row_inst: Node3D, length:Vector2, center: bool): 
	...
	var GB_vec = Vector2(z, x)
	
	if GB_vec in path: 
		path_nodes.append(ground_block)
